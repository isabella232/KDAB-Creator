...
#include <qtsupport/gammaraytool.h>
...
bool GammarayEngine::start()
{
    ...
    if (m_settings->useQtcGammaray()) {
        const QString qmakePath = ProjectExplorer::DebuggingHelperLibrary::findSystemQt(rc->environment());
        QtSupport::BaseQtVersion *version = QtSupport::QtVersionManager::instance()->qtVersionForQMakeBinary(qmakePath);
        if (!version) {
            emit outputReceived(tr("Could not find Qt Version for QMake binary %1").arg(qmakePath),
                                Utils::ErrorMessageFormat);
            finishOnError();
            return false;
        } else if (!version->hasGammaray()) {
            emit outputReceived(tr("The GammaRay Helper is not yet build for the Qt Version %1.\n"
                                   "Go to Options -> Qt4 to build the helper.").arg(version->displayName()),
                                Utils::ErrorMessageFormat);
            finishOnError();
            return false;
        } else {
            exe = version->gammarayTool();
        }
    }
