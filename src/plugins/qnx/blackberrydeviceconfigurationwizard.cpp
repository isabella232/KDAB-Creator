...
BlackBerryDeviceConfigurationWizard::BlackBerryDeviceConfigurationWizard(QWidget *parent) :
    RemoteLinux::ILinuxDeviceConfigurationWizard(parent)
{
...
RemoteLinux::LinuxDeviceConfiguration::Ptr BlackBerryDeviceConfigurationWizard::deviceConfiguration()
{
    ...
    RemoteLinux::LinuxDeviceConfiguration::Ptr configuration =
            RemoteLinux::LinuxDeviceConfiguration::create(m_setupPage->deviceName(),
                                                          QLatin1String(Constants::QNX_BB_OS_TYPE),
                                                          m_setupPage->deviceType(), RemoteLinux::PortList(), sshParams);

    configuration->setAttribute(QLatin1String(Constants::QNX_DEBUG_TOKEN_KEY), m_setupPage->debugToken());
...
void BlackBerryDeviceConfigurationWizard::accept()
{
    if (m_sshKeyPage->isGenerated()) {
        if (saveKeys())
            RemoteLinux::ILinuxDeviceConfigurationWizard::accept();
    } else {
        RemoteLinux::ILinuxDeviceConfigurationWizard::accept();
    }
}
