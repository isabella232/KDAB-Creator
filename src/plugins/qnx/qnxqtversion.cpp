...
QSet<QString> QnxQtVersion::supportedTargetIds() const
{
    QSet<QString> result;

    if (architecture() == X86)
        result << QLatin1String(Constants::QNX_QNX_X86_TARGET_ID);
    else if (architecture() == ArmLeV7)
        result << QLatin1String(Constants::QNX_QNX_ARMLEV7_TARGET_ID);
...
QMap<QString, QString> QnxQtVersion::environment() const
{
    ...
    environment.insertMulti(QLatin1String("PATH"), sdkPath() + QLatin1String("/host/win32/x86/usr/bin"));

    // TODO:
    //environment.insertMulti(QLatin1String("PATH"), QLatin1String("/etc/qnx/bin"));
#elif defined Q_OS_UNIX
    environment.insert(QLatin1String("QNX_CONFIGURATION"), QLatin1String("/etc/qnx"));
    environment.insert(QLatin1String(Constants::QNX_TARGET_KEY), sdkPath() + QLatin1String("/target/qnx6"));
    environment.insert(QLatin1String(Constants::QNX_HOST_KEY), sdkPath() + QLatin1String("/host/linux/x86"));

    environment.insertMulti(QLatin1String("PATH"), sdkPath() + QLatin1String("/host/linux/x86/usr/bin"));
    environment.insertMulti(QLatin1String("PATH"), QLatin1String("/etc/qnx/bin"));
