...
QnxDebugSupport::QnxDebugSupport(QnxRunConfiguration *runConfig, Debugger::DebuggerEngine *engine)
    ...
    , m_state(Inactive)
{
    QnxRunConfigurationFactory fact;
    QnxRunConfiguration *pdebugRunConfig = qobject_cast<QnxRunConfiguration *>(fact.clone(runConfig->target(), runConfig));

    pdebugRunConfig->setUseAlternateExecutable(true);
    pdebugRunConfig->setAlternateRemoteExecutable(QLatin1String("pdebug"));

    m_runner = new QnxApplicationRunner(pdebugRunConfig, this);
...
void QnxDebugSupport::startExecution()
{
    ...
    const QString remoteCommandLine = QString::fromLatin1("%1 pdebug %2").arg(m_runner->commandPrefix()).arg(m_port);
...
{
    if (m_engine)
        m_engine->handleRemoteSetupDone(m_port, -1);
}
...
void QnxDebugSupport::handleRemoteProcessFinished(qint64 exitCode)
{
    ...
    if (m_state == Debugging) {
        ...
        const QString errorMsg = tr("The pdebug process closed unexpectedly.");
